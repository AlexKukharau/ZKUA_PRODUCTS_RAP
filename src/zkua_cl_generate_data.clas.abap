CLASS zkua_cl_generate_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZKUA_CL_GENERATE_DATA IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

      DATA: lt_prod_grs  TYPE TABLE OF zkua_d_prod_grup,
          lt_phases    TYPE TABLE OF zkua_d_phase,
          lt_markets   TYPE TABLE OF zkua_d_market,
          lt_uom       TYPE TABLE OF zkua_d_uom.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zkua_d_prod_grup.
*   insert the new table entries
    INSERT zkua_d_prod_grup FROM TABLE @lt_prod_grs.

*   check the result
    SELECT * FROM zkua_d_prod_grup INTO TABLE @lt_prod_grs.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zkua_d_phase.
*   insert the new table entries
    INSERT zkua_d_phase FROM TABLE @lt_phases.

*   check the result
    SELECT * FROM zkua_d_phase INTO TABLE @lt_phases.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zkua_d_market.
*   insert the new table entries
    INSERT zkua_d_market FROM TABLE @lt_markets.

*   check the result
    SELECT * FROM zkua_d_market INTO TABLE @lt_markets.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zkua_d_uom.
*   insert the new table entries
    INSERT zkua_d_uom FROM TABLE @lt_uom.

*   check the result
    SELECT * FROM zkua_d_uom INTO TABLE @lt_uom.
    out->write( sy-dbcnt ).
